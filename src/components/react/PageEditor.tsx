...
import { loadDraft, saveDraft as saveDraftToStore, deleteDraft, autoSaveDraft } from '../../utils/draftStore';
...
function createSection(type: SectionType, order: number): Section {
  ...
  switch (type) {
    ...
    case 'cta':
      return { ...baseSection, type: 'cta', heading: '', buttonText: '', buttonLink: '', style: 'primary' };
  }
}

export default function PageEditor({ slug, token, onSave, onCancel }: PageEditorProps) {
  ...
  const loadPageFromGitHub = async () => {
    ...
    setLoading(true);
    try {
      const result = await fetchPageContent(slug, token);
  ...
  const moveSection = (index: number, direction: 'up' | 'down') => {
    ...
    if (targetIndex < 0 || targetIndex >= newSections.length || !newSections[index] || !newSections[targetIndex]) return;
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              value={page.slug}
              onChange={(e) => updatePage({ slug: generateSlug(e.target.value) })}
            ...
            {slug && <p className="mt-1 text-xs text-gray-500">Slug cannot be changed when editing</p>}
          </div>
          ...
          <div>
            ...
            <select
              value={page.status}
              onChange={(e) => updatePage({ status: e.target.value as 'draft' | 'published' })}
      ...
      <div className="mb-6">
        ...
        {showAddSection && (
          <div className="mb-4 p-4 bg-gray-50 border border-gray-300 rounded-lg">
            <p className="text-sm font-medium text-gray-700 mb-3">Select section type:</p>
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                ...
                <div className="text-xs text-gray-500">Large banner with title and CTA</div>
              </button>
              ...
              >
                ...
                <div className="text-xs text-gray-500">Paragraph content with optional heading</div>
              </button>
              ...
              >
                ...
                <div className="text-xs text-gray-500">Multiple cards in a grid layout</div>
              </button>
              ...
              >
                ...
                <div className="text-xs text-gray-500">Prominent button with heading</div>
              </button>
            </div>
          </div>
        )}
        ...
        {page.sections.length === 0 && (
          <div className="text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
            <p className="text-gray-600">No sections yet. Click "Add Section" to create content.</p>
          </div>
        )}
        ...
        {page.sections.map((section, index) => {
          ...
          switch (section.type) {
            case 'hero':
              return <HeroEditor key={section.id} {...props} section={section} />;
            case 'text':
              return <TextEditor key={section.id} {...props} section={section} />;
            case 'cardGrid':
              return <CardGridEditor key={section.id} {...props} section={section} />;
            case 'cta':
              return <CtaEditor key={section.id} {...props} section={section} />;
            default:
              return null;
          }
        })}

...
async function checkRepositoryAccess(token: string, username: string): Promise<{ authorized: boolean; reason?: string }> {
  const repoOwner = import.meta.env.PUBLIC_GITHUB_OWNER || import.meta.env.GITHUB_OWNER;
  const repoName = import.meta.env.PUBLIC_GITHUB_REPO || import.meta.env.GITHUB_REPO;
...
export default function GitHubAuth({ onAuthenticated }: GitHubAuthProps) {
  ...
  const REDIRECT_URI = typeof window !== 'undefined' ? window.location.origin + '/admin' : '';
    ...
    const handleCallback = async () => {
      ...
      if (code && storedVerifier) {
        ...
        try {
          ...
          const accessCheck = await checkRepositoryAccess(accessToken, userData.login);
  ...
  const handleOAuthLogin = async () => {
    if (!GITHUB_CLIENT_ID) {
      setError('GitHub OAuth not configured. Please set PUBLIC_GITHUB_CLIENT_ID in .env');
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full">
          ...
          {!showManualToken && (
            <>
              <button
                ...
                disabled={loading}
                className="w-full bg-gray-900 text-white py-3 px-4 rounded-lg font-semibold hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-3"
              >
        ...
        <div className="mt-6 p-4 bg-blue-50 rounded-lg">
          <p className="text-sm text-blue-900">
            <strong>Secure:</strong> {showManualToken ? 'Your token is' : 'Authentication is'} stored locally and only used to commit updates to GitHub.
          </p>
        </div>
      </div>
    </div>
